...
from app.config import UPLOAD_DIR, STATIC_DIR, get_settings, update_settings, verify_password
from app.database import (init_db, create_post, get_posts, get_post_by_id, update_post, delete_post,
                          create_session, session_is_valid, delete_session, save_oauth_state,
...
def create_session_token() -> str:
    ...
    create_session(token, (datetime.now(timezone.utc) + timedelta(days=7)).isoformat())
    return token

def verify_auth(authorization: Optional[str] = Header(None), request: Request = None):
    token = None
...
def api_login(req: LoginRequest, request: Request):
    ...
    if not (settings.get("app_password_hash") or settings.get("app_password")):
        raise HTTPException(status_code=503, detail="Chưa cấu hình mật khẩu. Hãy đặt APP_PASSWORD trong .env rồi khởi động lại.")
    if not settings.get("app_password_hash") and settings.get("app_password") == "admin123":
        raise HTTPException(status_code=503, detail="Mật khẩu mặc định đã bị vô hiệu hóa. Hãy đổi APP_PASSWORD trong .env thành mật khẩu mạnh rồi khởi động lại.")
    if not verify_password(req.password, settings):
        raise HTTPException(status_code=401, detail="Mật khẩu nội bộ không chính xác!")
    if settings.get("app_password") and not settings.get("app_password_hash"):
        update_settings({"app_password": req.password})
...
def api_auth_check(auth=Depends(verify_auth)):
...
@app.post("/api/posts/create", dependencies=[Depends(verify_auth)])
def api_create_post(req: PostCreateRequest):
    if not req.target_fb and not req.target_ig and not req.target_google:
        raise HTTPException(status_code=400, detail="Vui lòng chọn ít nhất một nền tảng (Facebook, Instagram hoặc Google Business).")
    ...
    try:
        scheduled_time = normalize_schedule(req.scheduled_time) if req.action == "schedule" else None
...
def api_get_posts(filter_type: Optional[str] = None):
    ...
    if filter_type == "scheduled":
        ...
        filtered = [p for p in all_posts if p["status"] in ["success", "partial_failed", "failed", "publishing"]]
    else:
        filtered = all_posts
...
@app.post("/api/maintenance/run", dependencies=[Depends(verify_auth)])
...
def api_get_settings():
    ...
    return {
        ...
        "host": s.get("host"),
        "port": s.get("port"),
        "has_password": bool(s.get("app_password") or s.get("app_password_hash")),
...
def api_save_settings(req: SettingsUpdateRequest):
    ...
    secret_fields = ["app_password", "fb_page_access_token", "imgbb_api_key", "gemini_api_key", "google_client_secret"]
    for field in secret_fields:
        val = updates.get(field)
        if val is None or not str(val).strip() or "..." in str(val) or "•" in str(val):
            updates.pop(field, None)
    if "app_password" in updates and len(updates["app_password"]) < 6:
        raise HTTPException(status_code=400, detail="Mật khẩu mới cần ít nhất 6 ký tự.")
    new_settings = update_settings(updates)
    return {"success": True, "settings": {k: v for k, v in new_settings.items() if k != "app_password"}}
...
@app.post("/api/meta/exchange-permanent-token", dependencies=[Depends(verify_auth)])
