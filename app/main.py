...
def api_create_post(req: PostCreateRequest, auth: dict = Depends(verify_auth)):
    if not req.fb_caption and not req.ig_caption and not req.google_caption and not req.threads_caption and not req.story_hook and not req.story_image:
...
def api_get_settings():
    ...
    return {
        ...
        "threads_token_masked": f"{s.get('threads_access_token')[:8]}...{s.get('threads_access_token')[-6:]}" if s.get("threads_access_token") and len(s.get("threads_access_token")) > 14 else (s.get("threads_access_token") or ""),
        "threads_app_id": s.get("threads_app_id", ""),
        "has_threads_app_secret": bool(s.get("threads_app_secret")),
        "host": s.get("host"),
        "port": s.get("port"),
...
def api_save_settings(req: SettingsUpdateRequest):
    ...
    secret_fields = ["app_password", "admin_password", "staff_password", "fb_page_access_token", "imgbb_api_key", "gemini_api_key", "google_client_secret", "threads_access_token", "threads_app_secret"]
...
# ─────────────────────────────────────────────────────────────
@app.get("/api/threads/status", dependencies=[Depends(verify_auth)])
...
def api_get_calendar_events():
    ...
    for p in posts:
        ...
        events.append({
            ...
            "target_threads": bool(p.get("target_threads")),
