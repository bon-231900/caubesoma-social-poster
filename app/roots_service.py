...
def _load_product_image(image_filename_or_url: str) -> Image.Image:
    ...
    if Path(image_filename_or_url).name == image_filename_or_url:
        if not local_path.is_file():
            try:
                from app.database import restore_media_file_if_missing
                restore_media_file_if_missing(image_filename_or_url)
            except Exception:
                pass
...
def download_and_fit_to_square_1_1(image_filename_or_url: str, output_size: int = 1080) -> str:
    ...
    try:
        ...
        register_media_file(out_filename, original_name="ROOTS Square", tags=["ROOTS", "Studio"])
    except Exception:
        pass
    return out_filename
...
def create_social_feed_creative(
    ...
    try:
        from app.media_service import register_media_file
        register_media_file(output_name, original_name=product_name or "ROOTS Creative", tags=["ROOTS", "Creative", aspect_ratio])
    except Exception:
        pass
    return output_name
